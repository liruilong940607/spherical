...
try:
    ...
    from spherical import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "spherical_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "spherical_cuda.lib")
        ):
            ...
            with Console().status(
                f"[bold yellow]spherical: Setting up CUDA with MAX_JOBS={os.environ['MAX_JOBS']} (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]spherical: No CUDA toolkit found. spherical will be disabled.[/yellow]"
        )
